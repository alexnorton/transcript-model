	...
	var Transcript = function (_TranscriptRecord) {
	  ...
	  _createClass(Transcript, [{
	    ...
	    value: function getText() {
	      return this.get('segments').reduce(function (text, segment) {
	        return text + '\n' + segment.getText();
	      }, '').trim();
	    }
	  }], [{
	    ...
	    value: function fromText(text) {
	      return new Transcript({
	        segments: _immutable2.default.List(text.trim().split('\n').map(function (line) {
	          return new _UntimedSegment2.default({
	            speaker: 0,
	            words: new _immutable2.default.List(line.split(' ').map(function (t) {
	              return new _Word2.default({ text: t });
	            }))
	          });
	        }))
	      });
